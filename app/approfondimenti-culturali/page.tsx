...
export default function CulturalInsightsPage() {
  ...
  // Placeholder for lessons - you'll add PDFs here later
  const lessons: CulturalLesson[] = [
    // Example: { id: 1, key: 'lesson1', pdfUrl: '/pdfs/lesson1.pdf', category: 'philosophy' },
  ];

  const categories = ['all', 'philosophy', 'history', 'wellness', 'spirituality'];
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center reveal">
            <h1 className="text-5xl md:text-6xl font-bold text-olive-800 mb-6">
              {t('culturalInsights.hero.title').split(' ').slice(0, -1).join(' ')}{' '}
              ...
              {filteredLessons.map((lesson, index) => (
                ...
                >
                  <div className="h-48 bg-gradient-to-br from-olive-100 to-olive-200 flex items-center justify-center group-hover:from-olive-200 group-hover:to-olive-300 transition-all duration-300">
                    <svg className="w-20 h-20 text-olive-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <div className="p-6">
                    ...
                    <p className="text-gray-600 mb-4">
                      {t(`culturalInsights.lessons.${lesson.key}.description`)}
                    </p>
                    <a
                      href={lesson.pdfUrl}
                      download
